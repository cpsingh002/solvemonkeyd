...
class HomeController extends Controller
{
     ...
     public function ProductImportstoreBike(Request $request)
    {
     ...
     public function ProductImportstorescotty(Request $request)
    {
        $fileMimes = array(
            'text/x-comma-separated-values',
            'text/comma-separated-values',
        ...
        if (!empty($_FILES['file']['name']) && in_array($_FILES['file']['type'], $fileMimes)) {
            ...
            while (($getData = fgetcsv($csvFile, 10000, ",")) !== FALSE) {
                ...
                $brand = Brand::where('name',$brand_name)->where('category_id','3')->where('subcategory_id','8')->first();
                if(isset($brand)){  
                    ...
                    $brand->slug = Str::slug($brand_name);
                    $brand->category_id  = 3;
                    $brand->subcategory_id  = 8;
                    $brand->image = 'solve-logo.png';
                    $brand->status = 1;
                    $brand->save();
                }

                $modelnumber = ModelNumber::where('name',$model_name)->where('brand_id',$brand->id)->where('category_id','3')->where('subcategory_id','8')->first();
                if(isset($modelnumber))
                {
                }else{
                    $modelnumber = new ModelNumber();
